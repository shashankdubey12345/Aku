...
export default function GiftBox() {
  ...
  const [boxHearts, setBoxHearts] = useState<{ id: number; x: number; scale: number; delay: number; rotate: number }[]>([]);
  ...
  const handleOpen = () => {
    ...
    // 2. Mobile haptic pulse
    triggerHaptic(250);
    
    // 3. Open state
    setIsOpen(true);

    // 4. Generate rising heart animation particles
    const heartsList = Array.from({ length: 18 }).map((_, i) => ({
      id: i,
      x: Math.random() * 140 - 70, // Horizontal dispersion
      scale: Math.random() * 0.7 + 0.6,
      rotate: Math.random() * 60 - 30, // Angle
      delay: i * 0.06 // Staggered entry
    }));
    setBoxHearts(heartsList);

    // 5. Trigger sweet confetti sparks
    confetti({
      particleCount: 50,
      spread: 60,
      origin: { y: 0.7 },
  ...
  return (
    <div className="py-12 px-4 w-full max-w-4xl mx-auto flex flex-col items-center select-none relative">
      ...
      <div className="relative w-full max-w-[340px] flex flex-col items-center justify-center min-h-[420px]">
        {/* Floating Heart eruption above box */}
        <div className="absolute top-[80px] w-full flex justify-center pointer-events-none z-30">
          {boxHearts.map((h) => (
            <motion.span
              key={h.id}
              initial={{ y: 80, opacity: 0, x: 0, scale: 0.1, rotate: 0 }}
              animate={{ 
                y: -180, 
                opacity: [0, 1, 1, 0], 
                scale: h.scale,
                x: h.x + (Math.sin(h.id) * 20),
                rotate: h.rotate
              }}
              transition={{ duration: 2.8, delay: h.delay, ease: 'easeOut' }}
              className="absolute text-rose-500 text-xl font-normal"
            >
              ❤️
            </motion.span>
          ))}
          ...
          {!isOpen ? (
            ...
            >
              ...
              <motion.div
                ...
                className="w-[180px] h-[55px] bg-gradient-to-r from-pink-400 via-pink-500 to-pink-400 rounded-lg relative z-25 flex justify-center shadow-md border-b-2 border-pink-600/30"
              >
            ...
            <motion.div
              ...
              className="w-full glass p-6 rounded-3xl border border-pink-200 dark:border-white/10 shadow-xl flex flex-col items-center gap-4 bg-white/95 dark:bg-zinc-900/95 relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-rose-500/5 dark:bg-rose-500/2 rounded-bl-full flex items-center justify-center -z-10 pointer-events-none">
                <Heart className="w-10 h-10 text-rose-500/10 fill-rose-500/5" />
              </div>
              ...
              <button
                onClick={handleReset}
                className="mt-2 text-xs font-extrabold text-zinc-400 dark:text-zinc-500 hover:text-rose-500 transition-colors uppercase tracking-widest cursor-pointer flex items-center gap-1.5"
              >
                <RotateCcw className="w-3.5 h-3.5" /> Wrap Up Again
              </button>
            </motion.div>
          )}
        </AnimatePresence>
